...
- **Volume ramp and region gating are configuration decisions** — see Suppression & opt-out below. Suppression gate + circuit breaker kept.
...
- **`presumed_silent_graduate` is a ONE-SHOT manual cohort, NOT a live-fed segment.** The scorer still routes PSG out (absent from the `qualified`/contact/worklist paths in `ceiling_poll.py`), so no new PSG flows automatically. This batch was enriched offline, filtered to the 63 with a real non-the platform edge-function API integration (Stripe/Resend/OpenAI/…), junk/placeholder-email filtered, email-deduped vs the live worklist+send_log, and **staged: 15 seeded, 46 held back** pending a bounce check on the first batch. To release the rest, upsert the remaining `psg_survivors.csv` rows.
...
- **`daily_cap` kept at 100** (a configuration choice). Note this is now a *sustained* ~50–60/day cold stream from `sender@example.com`, not a one-week backlog drain — the ceiling stream self-refills and the edge signal raises B_all_bot inflow. Steady-state reputation exposure accepted.
- `C_mover_fresh` deliberately left OUT (thinner/fresher signal).

## Load-bearing risk (stated, accepted, mitigated)
Cold-sending from the primary domain risks deliverability of **all** `your-domain.example` mail. Accepted by the operator. Mitigations: from-address is one config line (swap to a dedicated domain by editing config + DNS, no code change); flat conservative cap; circuit breaker; validation culls dead domains before they bounce.

## Architecture
```
engine_loop.sh (every 30 min) ── mkdir lock (whole-cycle) ── run_chain.sh
   ├─ ceiling_poll.py --scrape-sites   (discover + enrich + worklist upsert)   ┐ throttled to
   ├─ verify_queue.py --apply          (Bouncer MAILBOX check; only if verify_enabled)  ├ ~hourly via
   ├─ bounce_scan.py                   (bounces + reply opt-outs → suppression)  ┘ .last_poll
   └─ send_outreach.py                 (every fire)
        ├─ flock (send-level) · STOP · HALT · live-gate (dry_run/template_approved/copy)
        ├─ breaker check (DISABLED in the committed default since 2026-07-27 — breaker_enabled:false)
        ├─ adaptive throttle (trailing bounce rate scales the daily cap; stop tier writes HALT)
        ├─ select  (status blank · valid email · verified if require_verified · in-segment · not
        │           suppressed · not already-sent · in the recipient's local window · under cap)
        └─ per row: write-ahead pending → Gmail send → live log → mark status=sent  (+jitter,
                    throttled-cap/window re-check)
```
...
- **Pre-send mailbox verification** (`verify_queue.py`, `verify_enabled`) — the first line of defence since 2026-07-27. `validate_emails` proves a *domain* accepts mail; Bouncer proves the *mailbox* exists. Only `undeliverable` is terminal (→ `status=skipped`); `risky` / `unknown` / deliverable-on-an-accept-all-domain are recorded and stay sendable. Vendor error ≠ verdict: the run stops and rows stay unverified for the next cycle. Cull ceiling: >40% undeliverable in a batch (min 20) aborts and writes nothing. Chronically-`unknown` rows are re-checked at most every 7 days, so a greylisting mailbox cannot be re-billed hourly forever. Takes the sender's `flock` so a hand-run `--apply` cannot race a live send, and re-reads the worklist after the API window rather than rewriting a stale snapshot. Full rationale and port notes: `method/VERIFY-INTEGRATION.md`.
  - **"Nothing is mailed unverified" is only true with `require_verified:true`.** The chain runs verification before the send, but it verifies at most `verify_max_per_run` rows and skips the step entirely on a vendor error, so ordering alone does not guarantee it — it happens to hold today only because `worklist_upsert` sorts by segment rank and the sender's first rows land inside the limit. `require_verified` makes it structural: a row with a blank `verify_status` is not selectable. Turn it on together with `verify_enabled`.
- **Circuit breaker — DISABLED in the committed default 2026-07-27** (`breaker_enabled:false`). It halted on a trailing rate whose confidence interval was far wider than the gap between "fine" and "stop" (it tripped at 7/100 against a 6% threshold), and it **deadlocks by construction**: `breaker_reason()` runs *before* any send, so while halted the trailing window is frozen and no clean send can ever dilute it — clearing `HALT` re-trips at any elapsed time. Bounces are still scanned and hard-bounced addresses still suppressed, so nobody is re-mailed and the rate stays recorded in `bounces.csv`; only the automatic stop is off. `bounce_rate_halt` / `bounce_window` / `bounce_burst` are inert while disabled and kept so a rollback is one edit. Fail-safe: only a literal `false` disables it, so a config predating this change keeps its breaker.
  - **This is the committed default, not necessarily the running state.** The live machine runs an *uncommitted* `config.json` overlay (it carries `dry_run:false`, `template_approved:true`, the 4-segment list, `daily_cap`, and historically a widened `bounce_window`). Merging this change does **not** update that file — `breaker_enabled:false` has to be added to the live overlay by hand, or the breaker stays armed with no log line saying so.
  - **`STOP` is not the only brake, despite what is easy to assume.** Still automatic and still live: `HALT` (written by the auth-fatal guard on a dead OAuth token, and by the throttle's stop tier), the throttle stop tier itself, the daily cap, the per-recipient timezone window, suppression + already-sent dedup, the role/region/segment/`email_valid` filters, and the three-flip live gate. What is gone is *bounce-rate-driven halting* specifically.
  - **Bounce rate is not on the dashboard.** `build_metrics.py` renders Sent / Replies / Accounts only — no bounce, suppression, HALT or throttle state. With the breaker off, the throttle multiplier exists as one line per cycle in `chain.log` and nowhere else. Worth fixing before leaning on "it stays visible".
- **Adaptive throttle** (`adaptive_throttle`, `THROTTLE_TIERS` in `send_outreach.py`) — what replaces the halt. Trailing hard-bounce rate scales the daily cap instead of flipping a switch: ≥4% → 75%, ≥7% → 50%, ≥12% → 25%, ≥20% → stop. Below `throttle_min_n` (40) sends in the window the rate is noise and the cap stays full. Reputation harm scales with volume, so throttling is the proportionate response; only a genuinely bad list reaches zero. Note this is the *second* line of defence — it exists for what verification cannot catch: silent-quarantine gateways, mailboxes that die between verification and send, and Gmail returning 250 for every address.
...
breaker_enabled: false                      # bounce-rate halting OFF (2026-07-27). HALT/cap/window still stop it.
bounce_rate_halt: 0.06, bounce_window: 100  # breaker: rate guard — INERT while breaker_enabled:false
bounce_burst: [4, 10]                       # breaker: >=4 hard bounces in last 10 — INERT likewise
adaptive_throttle: true                     # bounce rate scales the cap (tiers in send_outreach.py)
throttle_window: 100, throttle_min_n: 40    # trailing window / minimum sample before it acts
verify_enabled: false, verify_max_per_run: 150   # Bouncer pre-send mailbox check (VERIFY-INTEGRATION.md)
require_verified: false                     # when true, a blank verify_status is not sendable
```
**`verify_max_per_run: 0` means UNLIMITED, not off.** Use `verify_enabled:false` to turn it off.
```
segments: ["B_all_bot"], require_email_valid: true
worklist: outreach/worklist_ceiling.csv
```

**Key for verification** — not in the repo, mode 600, in the **shared** secrets dir next to
`apollo.env` and the Google OAuth files. Both engines use the SAME key on purpose: Bouncer credits
are per-key, and a duplicate splits the usage accounting (a configuration choice).
```
